...
lister.getSections = async (subject_id, course_id) => {
    ...
    let url = "https://public.enroll.wisc.edu/api/search/v1/enrollmentPackages/"+termCode+"/"+subject_id+"/"+course_id+"/"
    ...
    */ 
    const sections = api_req.data.map( entry => {
        return {
            ...
            lecture_name: entry.sections.filter(x => x.type=="LEC")
            .map(x => {
